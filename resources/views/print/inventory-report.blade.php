...
<body class="bg-white {{ ($format == '58mm' || $format == '76mm') ? 'p-1' : 'p-8' }}" onload="window.print()">
    <div class="{{ ($format == '58mm' || $format == '76mm') ? 'font-mono text-xs max-w-['.$format.']' : 'w-full max-w-4xl mx-auto a4-font text-sm' }}">
        ...
        @elseif($tab === 'purchases')
            <div class="mb-4">
                <h3 class="font-bold uppercase text-xs text-gray-700 mb-2">2. Riwayat Pembelian Stok</h3>
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-gray-400 font-bold bg-gray-50">
        ...
        @elseif($tab === 'productions')
            <div class="mb-4">
                <h3 class="font-bold uppercase text-xs text-gray-700 mb-2">3. Riwayat Batch Repacking Produksi</h3>
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-gray-400 font-bold bg-gray-50">
                            ...
                            <th class="py-1.5 px-2">Hasil Produk</th>
                            ...
                            <tr>
                                ...
                                <td class="py-1.5 px-2">
                                    @foreach($prod->inputs as $in)
                                        <div>{{ $in->ingredient?->name }}: {{ number_format($in->quantity, 2, ',', '.') }}</div>
                                    @endforeach
                                </td>
        ...
        @elseif($tab === 'stock_logs')
            <div class="mb-4">
                <h3 class="font-bold uppercase text-xs text-gray-700 mb-2">4. Mutasi & History Stok Bahan</h3>
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-gray-400 font-bold bg-gray-50">
