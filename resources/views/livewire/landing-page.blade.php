<div class="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-primary-500 selection:text-white">
    ...
    <nav class="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-100 transition-all duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-3.5 group">
                ...
                @elseif($siteLogo)
                    <img src="{{ asset('storage/' . $siteLogo) }}" alt="{{ $storeName }}" class="w-11 h-11 object-cover rounded-xl shadow-xs group-hover:scale-105 transition-transform border border-slate-100">
                @else
            ...
            <div class="flex items-center gap-3">
                <a 
                    ...
                    class="px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-bold rounded-xl transition-all shadow-sm hover:shadow-md flex items-center gap-2"
                >
                ...
                <a 
                    href="{{ route('login') }}" 
                    class="px-4 py-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-100 text-sm font-semibold rounded-xl transition-colors hidden sm:flex items-center gap-1.5 border border-slate-200"
                    title="Akses Kasir / Admin"
                >
                    <x-lucide name="log-in" class="w-4 h-4" />
                    <span>Staff POS</span>
                </a>
            </div>
        </div>
    ...
    <!-- Hero Section -->
    <section id="beranda" class="relative bg-white pt-12 pb-20 md:pt-20 md:pb-28 border-b border-slate-100 overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
                <!-- Left Text Content -->
                <div class="lg:col-span-7 space-y-6 text-center lg:text-left">
                    <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-primary-50 border border-primary-100 text-primary-700 text-xs font-bold uppercase tracking-wider">
                        <span class="w-2 h-2 rounded-full bg-primary-600 animate-pulse"></span>
                        Cita Rasa Khas Malang
                    </div>

                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight leading-[1.15]">
                        Lezatnya <span class="text-primary-600">Bakso Malang</span> Spesial Daging Sapi Pilihan
                    </h1>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-2">
                        <a 
                            href="{{ route('menu.public') }}" 
                            class="w-full sm:w-auto px-8 py-4 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-2xl shadow-md hover:shadow-lg transition-all text-center flex items-center justify-center gap-2 text-base"
                        >
                        ...
                        <a 
                            href="#lokasi" 
                            class="w-full sm:w-auto px-8 py-4 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-2xl transition-all text-center border border-slate-200 text-base"
                        >
                            Lokasi & Jam Buka
                        </a>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 grid grid-cols-3 gap-4 max-w-lg mx-auto lg:mx-0 text-center lg:text-left">
                        ...
                        <div>
                            <p class="text-2xl font-extrabold text-slate-900">Fresh</p>
                            <p class="text-xs text-slate-500 font-medium">Bahan Setiap Hari</p>
                        </div>
                        <div>
                            <p class="text-2xl font-extrabold text-slate-900">Otentik</p>
                ...
                <div class="lg:col-span-5 relative flex justify-center">
                    <div class="relative w-full max-w-md bg-white rounded-3xl p-4 shadow-xl border border-slate-100">
                        @if($featuredProducts->first() && $featuredProducts->first()->image)
                            <div class="h-72 w-full rounded-2xl overflow-hidden bg-slate-100 relative">
                                <img src="{{ asset('storage/' . $featuredProducts->first()->image) }}" class="w-full h-full object-cover">
                                <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-slate-800 shadow-xs">
                                    ★ Menu Unggulan
                                </div>
                            </div>
                        @else
                            <div class="h-72 w-full rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 flex flex-col items-center justify-center text-slate-400">
                        ...
                        <!-- Floating Badge -->
                        <div class="absolute -bottom-5 -left-5 bg-white p-3.5 rounded-2xl shadow-lg border border-slate-100 flex items-center gap-3">
                            <div class="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center">
                                <x-lucide name="check-circle" class="w-6 h-6" />
                            </div>
                            <div>
                                <p class="text-xs font-extrabold text-slate-800">Buka Hari Ini</p>
                                <p class="text-[11px] text-slate-500 font-medium">{{ $storeHours }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section id="keunggulan" class="py-16 md:py-24 bg-slate-50 border-b border-slate-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-2xl mx-auto mb-14 space-y-3">
                <span class="text-xs font-bold text-primary-600 uppercase tracking-widest bg-primary-50 px-3.5 py-1 rounded-full border border-primary-100">
    ...
    <footer class="bg-slate-900 text-slate-400 py-12 border-t border-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3">
                    @if($logoWeb)
                        <img src="{{ asset('storage/' . $logoWeb) }}" class="w-10 h-10 object-cover rounded-xl border border-slate-700">
                    @elseif($siteLogo)
                        <img src="{{ asset('storage/' . $siteLogo) }}" class="w-10 h-10 object-cover rounded-xl border border-slate-700">
                    @else
                    ...
                    @endif
                    <div>
                        <span class="text-lg font-bold text-white block leading-none">{{ $storeName }}</span>
                        <span class="text-xs text-slate-500">Cita Rasa Otentik Bakso Malang</span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-6 text-xs font-semibold">
                    ...
                    <a href="{{ route('menu.public') }}" class="hover:text-white transition-colors">Menu Publik</a>
                    <a href="{{ route('login') }}" class="hover:text-white transition-colors">Staff Login</a>
                </div>

                <!-- Copyright -->
                <div class="text-xs text-slate-500">
                    &copy; {{ date('Y') }} {{ $storeName }}. Hak Cipta Dilindungi.
                </div>
            </div>
        </div>
    </footer>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full z-10 overflow-hidden relative flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 overflow-y-auto space-y-4 custom-scroll flex-1">
                    <div>
                        ...
                        @if($selectedProduct->description)
                            <p class="text-sm text-slate-600 mt-2 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <div class="p-4 border-t bg-slate-50 flex justify-end">
                    <button 
                        wire:click="closeProductModal" 
                        class="px-6 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-800 text-sm font-bold rounded-xl transition-colors"
                    >
                        Tutup
                    </button>
                </div>
