<div class="relative">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500">Konversi bahan mentah menjadi produk siap jual (bakso porsi/biji) & hitung HPP otomatis</p>
        </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Tanggal Produksi</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Bahan Terpakai</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Hasil Produk Jadi</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Total Biaya Produksi</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-xs text-gray-800 font-medium">
                            @foreach($p->outputs->take(2) as $out)
                                <div>+ {{ $out->product?->name }}: {{ number_format($out->quantity, 0) }} Pcs</div>
                            @endforeach
                            @if($p->outputs->count() > 2)
                                <span class="text-gray-400">+{{ $p->outputs->count() - 2 }} produk lainnya</span>
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            <i data-lucide="utensils-crossed" class="w-12 h-12 mx-auto mb-3 text-gray-300"></i>
                            <p>Belum ada riwayat produksi dapur</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden border border-gray-100 max-h-[92vh] flex flex-col">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div>
                    ...
                    <p class="text-xs text-gray-500">Pilih bahan mentah yang digunakan & produk jadi yang dihasilkan.</p>
                </div>
            ...
            <form wire:submit.prevent="save" class="p-6 space-y-6 overflow-y-auto flex-1 custom-scroll">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input wire:model="production_code" type="text" class="w-full text-sm border border-gray-200 rounded-lg p-2.5 font-mono">
                    </div>
                    <div>
                ...
                <!-- Step 2: Outputs (Hasil Produk Jadi) -->
                    ...
                    <div class="flex justify-between items-center border-b border-emerald-200/60 pb-2">
                        <div>
                            <h4 class="text-sm font-bold text-emerald-900 flex items-center gap-1.5">
                                ...
                                2. Hasil Produk Jadi Dapur (Ditambahkan ke Stok POS)
                            </h4>
                        </div>
                        <button type="button" wire:click="addOutputItem" class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 hover:text-emerald-800">
                            <i data-lucide="plus" class="w-4 h-4"></i>
                            <span>Tambah Produk Hasil</span>
                        </button>
                    </div>
                    ...
                    @foreach($outputItems as $index => $item)
                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end p-3 bg-white rounded-lg border border-emerald-200/60">
                            <div class="sm:col-span-5">
                                ...
                                <select wire:model.live="outputItems.{{ $index }}.product_id" class="w-full text-xs border border-gray-200 rounded-lg p-2 bg-white">
                                    <option value="">-- Pilih Produk Jadi POS --</option>
                                    @foreach($products as $prd)
                                        <option value="{{ $prd->id }}">{{ $prd->name }} (Stok saat ini: {{ $prd->stock }})</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="sm:col-span-3">
                                <label class="block text-xs font-medium text-gray-600 mb-1">Hasil Biji/Porsi (Qty)</label>
                                <input wire:model.live="outputItems.{{ $index }}.quantity" type="number" min="1" class="w-full text-xs border border-gray-200 rounded-lg p-2">
                            </div>

                            <div class="sm:col-span-3">
                                <label class="block text-xs font-medium text-gray-600 mb-1">Calculated HPP / Item</label>
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="save" class="flex-1 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg shadow-sm flex items-center justify-center gap-2">
                        <span wire:loading.remove wire:target="save">Proses & Update Stok POS</span>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div>
                    <h3 class="font-bold text-gray-800">Detail Batch Produksi: {{ $selectedProduction->production_code }}</h3>
                    <p class="text-xs text-gray-500">Tanggal: {{ $selectedProduction->production_date->format('d M Y') }}</p>
                </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <!-- Outputs Table -->
                <div>
                    <h4 class="text-xs font-bold uppercase text-emerald-800 mb-2">2. Hasil Produk Jadi:</h4>
                    <table class="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr class="bg-emerald-50 border-b text-emerald-900 font-semibold">
                                <th class="py-2 px-3">Produk Jadi</th>
                                <th class="py-2 px-3 text-center">Hasil (Pcs/Porsi)</th>
                                <th class="py-2 px-3 text-right">HPP Terhitung / Item</th>
                                <th class="py-2 px-3 text-right">Subtotal</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach($selectedProduction->outputs as $out)
                                <tr>
                                    <td class="py-2 px-3 font-medium">{{ $out->product?->name }}</td>
                                    <td class="py-2 px-3 text-center font-bold text-emerald-700">{{ number_format($out->quantity, 0) }} Pcs</td>
