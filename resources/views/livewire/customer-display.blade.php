<div class="h-screen bg-gray-50 flex flex-col overflow-hidden relative" x-data="{
         cart: @js($cart),
     ...
     }" x-init="initQrisTimer()" @cart-data-broadcast.window="
    ...
    @endphp
    <div
        class="bg-white shadow-sm border-b px-6 py-4 flex justify-between items-center transition-all duration-300 z-10 flex-shrink-0">
        ...
        <div class="flex items-center gap-4">
            <span
                class="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-2 rounded-full font-bold text-base shadow-2xs"
                x-show="customerName" x-text="'Pelanggan: ' + customerName">
            </span>

            <!-- Menu Toggle Button for Customer Display -->
            <button @click="showMenuModal = !showMenuModal"
    ...
    <div x-show="Object.keys(cart).length === 0"
        class="flex-1 flex flex-col items-center justify-center bg-white text-center p-8 animate-fade-in overflow-hidden">
        <div
            class="w-56 h-56 bg-primary-50/60 rounded-full flex items-center justify-center mb-6 shadow-inner border border-primary-100/50">
        ...
        <p class="text-xl text-gray-500 font-light max-w-md">Silakan lihat menu favorit kami dan pilih sesuai selera
            Anda</p>
    ...
    <div x-show="Object.keys(cart).length > 0" class="flex-1 flex flex-col md:flex-row overflow-hidden"
        style="display: none;">
        <!-- LEFT: Items List (60% on desktop) -->
        <div id="items-container"
            class="w-full md:w-3/5 p-4 overflow-y-auto border-b md:border-b-0 md:border-r border-gray-200 bg-white custom-scroll scroll-smooth relative">
            <div class="space-y-3 pb-4">
                <template x-for="(item, cartKey) in cart" :key="cartKey">
                    <div
                        class="flex items-start gap-3 p-3.5 rounded-xl border border-gray-100 bg-gray-50/80 hover:bg-white hover:shadow-md transition-all group">
                        <!-- Quantity Badge -->
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1.5 flex flex-wrap gap-1"
                                x-show="item.modifiers && Object.keys(item.modifiers).length > 0">
                                <template x-for="(mod, modKey) in item.modifiers" :key="modKey">
                                    <span
                                        class="text-[11px] bg-white border border-gray-200 px-2 py-0.5 rounded-md text-gray-600 flex items-center gap-1">
                                        <span x-text="'+ ' + mod.name"></span>
                                        <span class="font-bold text-gray-500" x-show="mod.price > 0"
                        ...
                        <div class="text-right">
                            <span class="text-lg font-extrabold text-gray-800"
                                x-text="'Rp ' + formatNumber(item.subtotal)"></span>
                        </div>
                    </div>
                </template>
            </div>
        ...
        <div class="w-full md:w-2/5 bg-gray-50 flex flex-col shadow-inner overflow-hidden">
            ...
            <div class="flex-1 p-8 flex flex-col justify-center space-y-6 overflow-y-auto">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-5xl font-black text-primary-600 tracking-tight"
            ...
            <div class="bg-white p-8 border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] flex-shrink-0">
                <div x-show="paidAmount > 0" class="space-y-4">
                    ...
                    <div
                        class="flex justify-between items-center p-4 rounded-xl text-green-800 bg-green-100 shadow-inner">
                        <span class="text-2xl font-bold uppercase">Kembalian</span>
                        <span class="text-4xl font-extrabold"
                            x-text="'Rp ' + formatNumber(Math.max(0, changeAmount))"></span>
                    </div>
                </div>
                <div x-show="!paidAmount || paidAmount <= 0"
                    class="text-center text-gray-400 py-4 flex flex-col items-center gap-2">
    ...
    <div x-show="showQris && qrisCodeUrl" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md overflow-hidden">
            ...
            <div class="p-6 text-center space-y-5">
                <!-- Total Tagihan -->
                <div>
                    <span class="text-xs font-semibold text-gray-400 uppercase tracking-wide block">Total
                        Pembayaran</span>
                    <span class="text-3xl font-extrabold text-gray-900"
                        x-text="'Rp ' + formatNumber(qrisAmount || total)"></span>
                </div>

                <!-- QR Code Box -->
                <div class="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm mx-auto w-fit">
                    <img x-show="qrisCodeUrl" :src="qrisCodeUrl || ''" alt="Kode QRIS Pembayaran"
                        class="w-56 h-56 mx-auto object-contain rounded-lg">
                </div>

                <!-- Live Status -->
                <div x-show="qrisCountdown > 0 && !qrisStatusText"
                    class="inline-flex items-center gap-1.5 bg-blue-50 text-primary-600 text-xs font-semibold px-3 py-1.5 rounded-full">
                ...
                <div class="space-y-1.5">
                    <div class="text-xs text-gray-500">
                        ...
                        <span class="font-mono font-bold text-gray-900 text-sm"
                            x-text="formatCountdown(qrisCountdown)"></span>
                    </div>
                    ...
                    <div x-show="qrisStatusText"
                        class="text-xs font-semibold text-primary-600 bg-blue-50 p-2.5 rounded-xl mt-2"
                        x-text="qrisStatusText">
                    </div>
                </div>

                <!-- Tombol HANYA Cek Status Pembayaran (Tanpa Tombol Batal) -->
                <div class="pt-2">
                    <button @click="checkQrisStatus()" :disabled="isCheckingQris"
    ...
    <div x-show="showMenuModal" x-cloak class="fixed inset-0 z-50 overflow-hidden flex justify-end"
        ...
        <div class="relative w-full max-w-4xl bg-gray-50 h-full shadow-2xl flex flex-col z-10 transform transition-transform duration-300"
            x-show="showMenuModal" x-transition:enter="transition ease-out duration-300 transform"
            ...
            <div class="p-6 flex-1 overflow-y-auto custom-scroll">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    @foreach($products as $product)
                        <div x-show="(selectedCategory === null || selectedCategory === {{ $product->category_id }}) && ('{{ addslashes(strtolower($product->name)) }}'.includes(searchQuery.toLowerCase()))"
                            ...
                            <div class="relative h-44 w-full bg-gray-100 overflow-hidden">
                                ...
                                <div class="absolute top-2.5 left-2.5">
                                    <span
                                        class="bg-black/60 backdrop-blur-md text-white text-[11px] font-bold px-2.5 py-0.5 rounded-full">
                                ...
                                @if(!$product->isAvailable())
                                    <div
                                        class="absolute inset-0 bg-black/50 backdrop-blur-[1px] flex items-center justify-center">
                                        <span
                                            class="bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Habis</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="p-4 flex-1 flex flex-col justify-between space-y-2">
                                <div>
                                    <h3
                                        class="font-bold text-gray-800 text-base leading-snug line-clamp-1 group-hover:text-primary-600 transition-colors">
                                        {{ $product->name }}
                                    </h3>
                                ...
                                <div class="pt-2 border-t border-gray-100 flex items-center justify-between">
                                    ...
                                    <button
                                        class="p-2 bg-primary-50 text-primary-600 rounded-lg group-hover:bg-primary-600 group-hover:text-white transition-colors">
    ...
    <div x-show="detailProduct !== null" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        <div class="fixed inset-0 bg-black/70 backdrop-blur-sm" @click="detailProduct = null"></div>

        <div
            class="bg-white rounded-3xl shadow-2xl max-w-md w-full z-10 overflow-hidden relative flex flex-col max-h-[85vh] animate-fade-in">
            <button @click="detailProduct = null"
                class="absolute top-3 right-3 z-20 p-2 bg-black/50 hover:bg-black/70 text-white rounded-full transition-colors">
            ...
            <div class="h-60 w-full bg-gray-100 relative shrink-0">
                ...
                <div class="absolute bottom-3 left-3" x-show="detailProduct && detailProduct.category">
                    <span class="bg-primary-600 text-white text-xs font-bold px-3 py-1 rounded-full"
                        x-text="detailProduct?.category?.name"></span>
                </div>
            </div>
            ...
            <div class="p-6 overflow-y-auto space-y-4 flex-1">
                <div class="flex justify-between items-start gap-4">
                    ...
                    <span class="text-2xl font-extrabold text-primary-600 shrink-0"
                        x-text="'Rp ' + formatNumber(detailProduct?.price)"></span>
                </div>
            ...
            <div class="p-4 border-t bg-gray-50 flex justify-end">
                <button @click="detailProduct = null"
                    class="px-6 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm font-bold rounded-xl transition-colors">
                    Tutup
                </button>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            // Cari component ID Livewire v3 dari DOM
            const getComponent = () => {
            ...
            const applyStateToAlpine = (component) => {
                const root = document.querySelector('[x-data]');
                if (!root || !root.__x || !component) return;

                // Guard: jangan timpa data yang lebih baru
                if ((component.get('lastUpdatedAt') || 0) < root.__x.$data.lastUpdatedAt) {
                    return;
                }
                root.__x.$data.lastUpdatedAt = component.get('lastUpdatedAt');
                root.__x.$data.cart = component.get('cart');
                root.__x.$data.subtotal = component.get('subtotal');
                root.__x.$data.taxAmount = component.get('taxAmount');
                root.__x.$data.total = component.get('total');
                root.__x.$data.paidAmount = component.get('paidAmount');
                root.__x.$data.changeAmount = component.get('changeAmount');
                root.__x.$data.customerName = component.get('customerName');
                root.__x.$data.cashierName = component.get('cashierName');
                root.__x.$data.qrisOrderId = component.get('qrisOrderId');
                root.__x.$data.qrisCodeUrl = component.get('qrisCodeUrl');
                root.__x.$data.qrisExpiresIn = component.get('qrisExpiresIn');
                root.__x.$data.qrisExpiresAtMs = component.get('qrisExpiresAtMs');
                root.__x.$data.showQris = component.get('showQris');
            ...
            const syncFromServer = () => {
                ...
                component.$refresh().then(() => {
                    applyStateToAlpine(getComponent());
                });
            };
